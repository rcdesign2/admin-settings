...
class Serializer {
    ...
    public function save() {
        ...
        if ( null !== wp_unslash( $_POST['rc-message'] ) ) {

            $value = sanitize_text_field( $_POST['rc-message'] );
            update_option( 'rc-custom-data', $value );

        }
        if ( null !== wp_unslash( $_POST['rc-insta-count'] ) ) {

            $value = sanitize_text_field( $_POST['rc-insta-count'] );
            update_option( 'rc-insta-count', $value );
    ...
    private function has_valid_nonce() {
        ...
        if ( ! isset( $_POST['rc-custom-message'] ) ) { // Input var okay.
            return false;
        }

        $field  = wp_unslash( $_POST['rc-custom-message'] );
        $action = 'rc-settings-save';
